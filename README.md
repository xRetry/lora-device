...
## Grafana
Grafana is a leading open-source platform for monitoring and observation, widely used to visualize time-series data. It can create dynamic dashboards that are essential for visualizing data patterns and extracting actionable insights. Grafana can integrate with a variety of data sources including InfluxDB, Prometheus, and MySQL, among others. 

Setting up the connection from InfluxDB to Grafana has been done according to the following [link](https://docs.influxdata.com/influxdb/cloud/tools/grafana/?t=InfluxQL). Remember to set the query language to "InfluxQL" and to define the URL with the respective container-name to ensure that both services are able to connect. Further, an API-token, generated by some user in the graphical web-interface of InfluxDB, has to be provided. Finally, the bucket-name, username, and password were stated. Define "GET" as the HTTP method, as we are only sending requests for the retrieval of measurement data. 

After clicking "Save & test", the connection is established and we were able to write queries and display the visualizations on a dashboard. 

When designing dashboards, the admin user also has the option to create user via the grahical user interface. So in contrast to the InfluxDB Cloud UI, the creation and managment of users is much easier. Grafana predefined a set of different user privileges, such as "Admin", "Editor", and "Viewer". These users require email-adresses for their creation, hence only the admin and an editor user were created. New users can be easily defined at `Configuration > Users`.
